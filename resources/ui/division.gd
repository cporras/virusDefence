...
func setResult():
	randomize();
	num2 = round (rand_range(2,5));
	num1 = num2 * round (rand_range(1,20));
	...
	fakeResult2 = round (rand_range(1,num1));
...
func _on_result_pressed():
		...
		get_node("/root/game").goodResultAction("division");

func _on_fakeResult_pressed():
		print("Fake Result");
		get_node("/root/game").fakeResultAction("division");
