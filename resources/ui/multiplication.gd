...
func setResult():
	randomize();
	num1 = round (rand_range(1,10));
	num2 = round (rand_range(1,10));
	result = num1*num2;
	if(rand_range(0,2)<1):
		fakeResult1 = result + round (rand_range(1,10));
	else:
		fakeResult1 = result - round (rand_range(1,10));
	
	fakeResult2 = round (rand_range(1,100));
...
func _on_result_pressed():
		...
		get_node("/root/game").goodResultAction("multiplication");

func _on_fakeResult_pressed():
		print("Fake Result");
		get_node("/root/game").fakeResultAction("multiplication");
